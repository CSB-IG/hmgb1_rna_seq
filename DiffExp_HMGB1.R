...
mynoiseqbio.deg <- degenes(mynoiseqbio, q=1-(1e-16))
# [1] "1294 differentially expressed features"

det <- nrow(mynoiseqbio.deg)
##visualizacion
pdf("NoiseqbioNormFull.pdf", width = 7*3, height = 3*3)
par(mfrow = c(1,2))
DE.plot(mynoiseqbio, q = 1-(1e-16), graphic = "expr", log.scale = FALSE)
DE.plot(mynoiseqbio, q = 1-(1e-16), graphic = "MD", log.scale = FALSE)
dev.off()
# [1] "1448 differentially expressed features"
write.csv(mynoiseqbio.deg, file = 'noiseqbioDEGNormalizadosFull.csv')
